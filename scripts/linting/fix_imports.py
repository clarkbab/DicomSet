...
def _find_module_import(
    name: str,
    file_path: Path,
) -> Optional[Tuple[int, str]]:
    """Search for a *module* (file or package) called *name* in the project.

    Returns ``(level, parent_module)`` so the caller can build
    ``from <dots><parent_module> import <name>``, or ``None`` if not found.

    For example, if *name* is ``config`` and ``config.py`` lives in the
    package root, this might return ``(2, '')`` for a file two levels deep,
    meaning ``from .. import config``.
    """
    pkg_root = file_path.parent
    while (pkg_root.parent / '__init__.py').exists():
        pkg_root = pkg_root.parent

    # Look for ``name.py`` or ``name/__init__.py`` under the package root.
    candidates: List[Path] = []
    for parent_dir in sorted({pkg_root} | {p.parent for p in pkg_root.rglob('__init__.py')}):
        if any(skip in parent_dir.parts for skip in SKIP_PATTERNS):
            continue
        mod_file = parent_dir / f'{name}.py'
        pkg_init = parent_dir / name / '__init__.py'
        if mod_file.exists() and mod_file != file_path:
    ...
    # Prefer the closest candidate (shortest path distance).
    from_parts = list(file_path.parent.parts)

    def _distance(mod_path: Path) -> int:
        # The parent directory of the module is where the ``from`` import
        # points to.
        mod_parent = list(mod_path.parent.parts)
        if mod_path.name == '__init__.py':
            # Package — the parent of the package dir.
            mod_parent = list(mod_path.parent.parent.parts)
        common = 0
        for a, b in zip(from_parts, mod_parent):
            if a == b:
                common += 1
            else:
                break
        return (len(from_parts) - common) + (len(mod_parent) - common)

    best = min(candidates, key=_distance)

    # Compute level and parent module dotted path.
    # For ``from <dots><parent> import name``, the parent is the
    # directory containing the module file (or the grandparent for
    # packages).
    if best.name == '__init__.py':
        target_parent_parts = list(best.parent.parent.parts)
    else:
        target_parent_parts = list(best.parent.parts)

    common = 0
    for a, b in zip(from_parts, target_parent_parts):
        if a == b:
            common += 1
        else:
            break
    ...
    remaining = target_parent_parts[common:]
    parent_module = '.'.join(remaining)

    return (level, parent_module)
...
def _public_names_from_file(module_path: Path) -> Set[str]:
    ...
    for node in ast.iter_child_nodes(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            ...
            elif is_init and (node.level or 0) > 0:
                # Only relative imports in __init__.py count as
                # re-exports.  Regular modules import names for their
                # own use, not to re-export them.
                for alias in node.names:
                    n = alias.asname if alias.asname else alias.name
                    if not n.startswith('_'):
                        names.add(n)
        elif is_init and isinstance(node, ast.Import):
            # ``import X`` or ``import X as Y`` contributes a public
            # name only in __init__.py (re-export pattern).
            for alias in node.names:
...
def sort_imports(source: str, file_path: Path) -> Tuple[str, int]:
    ...
    if is_init:
        ...
        for imp in expanded:
            ...
            # For relative imports, also verify the source module actually
            # provides the name — names imported from stdlib/third-party
            # by the source module (e.g. ``Dict`` from ``typing``) are
            # not its public names and should not be kept here.
            available: Optional[Set[str]] = None
            mod_dir: Optional[Path] = None
            if imp.is_relative:
                mod_path = _resolve_module_path(imp.full_module, file_path)
                if mod_path:
                    available = _public_names_from_file(mod_path)
                    # For packages, submodules are also valid import targets.
                    if mod_path.name == '__init__.py':
            ...
            for name, asname in imp.names:
                ...
                if available is not None and name not in available:
                    # Check if name is a submodule of the package.
                    if mod_dir is None or not (
                        (mod_dir / f'{name}.py').exists()
                        or (mod_dir / name / '__init__.py').exists()
                    ):
                        continue
    ...
    if not is_init:
        ...
        if missing:
            ...
            # Check stdlib / third-party modules FIRST so that standard
            # names (e.g. ``Dict``, ``List`` from ``typing``) are not
            # mistakenly attached to relative imports that happen to
            # re-export them.
            for imp in cleaned:
            ...
            # If names are still missing, check whether they belong to a
            # stdlib / third-party module that isn't imported yet.  Create
            # a new import line for such modules (e.g. ``from typing
            # import Dict, List``).
            if missing:
                _stdlib_candidates = ['typing', 'collections.abc', 'collections', 'os', 'pathlib', 'functools', 'itertools']
                for mod_name in _stdlib_candidates:
                    if mod_name not in module_available:
                        module_available[mod_name] = _public_names_from_module(mod_name)
                    to_add = missing & module_available[mod_name]
                    if not to_add:
                    ...
                    cleaned.append(_Import(
                        ...
                        level=0,
                        module=mod_name,
                        names=[(n, None) for n in to_add],
                        raw='',
                    ))
                    missing -= to_add

            # Then check relative (project-internal) imports for any
            ...
            # Check if any remaining missing names are module imports
            # (e.g. ``config`` from ``config.py``).  These need a
            # ``from <parent> import <module_name>`` style import.
            if missing:
                still_missing: Set[str] = set()
                for name in missing:
                    result = _find_module_import(name, file_path)
                    if result is not None:
                        level, parent_mod = result
                        # The circular-import check should target the
                        # actual module being imported, not the parent
                        # package.  E.g. for ``from .. import config``
                        # check ``..config``, not ``..``.
                        mod_key = '.' * level + (parent_mod + '.' + name if parent_mod else name)
                        if _would_cause_circular_import(file_path, mod_key):
                            continue
                        # Try to extend an existing import from the same parent.
                        found = False
                        for i, imp in enumerate(cleaned):
                            if (imp.is_from
                                    and imp.level == level
                                    and (imp.module or '') == parent_mod):
                                new_names = list(imp.names) + [(name, None)]
                                cleaned[i] = _Import(
                                    alias=None,
                                    is_from=True,
                                    level=imp.level,
            ...
            # Search the project for the defining module of any remaining
            # missing names (functions, classes, constants defined in other
            # files that aren't yet imported at all).
            if missing:
                still_missing2: Set[str] = set()
                for name in missing:
                    result = _find_defining_module(name, file_path)
                    if result is not None:
                        level, mod_dotted = result
                        full_key = '.' * level + mod_dotted
                        # Check for circular import.
                        if _would_cause_circular_import(file_path, full_key):
                            continue
    ...
    # Separate TYPE_CHECKING imports from regular imports.
    # External TC imports go right after ``from typing import`` (in the
    # external section).  Relative TC imports go after all relative imports.
    ext_regular = [imp for imp in external if not imp.type_checking]
    ext_tc = [imp for imp in external if imp.type_checking]
    rel_regular = [imp for imp in relative if not imp.type_checking]
    rel_tc = [imp for imp in relative if imp.type_checking]

    for imp in ext_regular:
        new_lines.append(_format_import(imp))
    if ext_tc:
    ...
    if (ext_regular or ext_tc) and (rel_regular or rel_tc):
        new_lines.append('')
    for imp in rel_regular:
        new_lines.append(_format_import(imp))
    if rel_tc:
        new_lines.append('if TYPE_CHECKING:')
        for imp in rel_tc:
