...
class NiftiCtSeries(NiftiImageSeries):
    def __init__(
        ...
        for e in IMAGE_EXTENSIONS:
            ...
            if os.path.exists(fpath):
                filepath = fpath
        if filepath is None:
            raise ValueError(f"No CT series '{self._id}' found for study '{self._study.id}'. Filepath: {basepath}, with extensions {IMAGE_EXTENSIONS}.")
    ...
    def dicom(self) -> DicomCtSeries:
        ...
        assert len(index) == 1, f"Expected 1 index entry for DICOM CT series '{self._id}', but found {len(index)}. Index: {index}"
        row = index.iloc[0]
