...
class NiftiDoseSeries(NiftiImageSeries):
    def __init__(
        ...
        for e in IMAGE_EXTENSIONS:
            ...
            if os.path.exists(fpath):
                filepath = fpath
        if filepath is None:
            raise ValueError(f"No nifti dose series found for study '{self.__study.id}'. Filepath: {basepath}, with extensions {IMAGE_EXTENSIONS}.")
    ...
    def dicom(self) -> DicomRtDoseSeries:
        ...
        index = index[(index['dataset'] == self.__dataset.id) & (index['patient-id'] == self.__patient.id) & (index['study-id'] == self.__study.id) & (index['series-id'] == self.__id) & (index['modality'] == 'dose')].drop_duplicates()
        assert len(index) == 1, f"Expected 1 index entry for DICOM dose series '{self.__id}', but found {len(index)}. Index: {index}"
        row = index.iloc[0]
