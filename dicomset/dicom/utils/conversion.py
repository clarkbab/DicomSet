...
def convert_to_nifti(
    ...
    study_sort: Callable[DicomStudy, int] | None = None,
    ) -> None:
    start = time()
    ...
    filepath = os.path.join(nifti_set.path, 'index.csv')
    if recreate_dataset or not os.path.exists(filepath):
    ...
    if anonymise_patients:
        if recreate_dataset or len(index) == 0:
            start_idx = 0
        else:
            existing_pat_ids = index['patient-id'].unique()
            start_idx = list(sorted(int(i.split('_')[1]) for i in existing_pat_ids))[-1] + 1
            logger.warning(f"Existing dataset has {len(existing_pat_ids)} patients, adding new patients starting from {start_idx}.")
    ...
    for p, ap in tqdm(zip(resolved_pat_ids, anon_pat_ids), total=len(resolved_pat_ids)):
        ...
        for s in studys:
            ...
            if not study.has_ct:
                logger.warning(f"Skipping study {study} due to no CT series. CT is required for RTSTRUCT reference geometry.")
                continue
            ...
            for sr in rtstruct_serieses:
                ...
                if region_id is not None:
                    ...
                    regions_data, regions_ids = series.regions_data(region_id=region_id, regions_ignore_missing=True, return_regions=True)
                ...
                if landmark_id is not None:
                    lm_df = series.landmarks_data(landmark_id=landmark_id, show_ids=False)
